...
export const getDefaultCompanions = async () => {
    ...
    try {
        const {data, error} = await supabase.from("default_companions").select();

        console.log("Data:", data);
        console.log("Error:", error);

        if (error) {
            throw new Error(error?.message || "Failed to get default companions");
        }
...
export const getCompanion = async (id: string) => {
    ...
    const {data: defaultCompapanionsData, error: defaultCompanionsError} =
        await supabase.from("default_companions").select().eq("id", id);

    if (defaultCompanionsError) {
        throw new Error(
            defaultCompanionsError?.message || "Failed to get companion"
        );
    }
